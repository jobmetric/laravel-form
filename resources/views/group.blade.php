<div class="border border-1 p-5 pb-n5 mb-10">
    <span class="fs-3 fw-bold text-highlight" data-label-colored="{{ mb_substr(trans($field->label), 0, 2) }}" data-label-noncolored="{{ mb_substr(trans($field->label), 2) }}"></span>
    ...
    @foreach($field->customFields as $index => $customField)
        @php
            /**
             * @var \JobMetric\CustomField\CustomField $customField
             */
            $classParent = 'mb-10';
            if ($index == $count - 1) {
                $classParent = '';
        ...
        @endphp
        {!! $customField->render($value, classParent: $classParent, hasErrorTagForm: true) !!}
    @endforeach
</div>
